...
def save_data(proposicoes):
    """Salva a lista de proposições em formato JSON."""
    with open(DATA_FILE, "w", encoding="utf-8") as f:
        json.dump(proposicoes, f, ensure_ascii=False, indent=4)

def format_proposicao(prop, detalhes):
    """Padroniza o objeto da proposição conforme os requisitos."""
    return {
...
def coletar():
    ...
    # Garante que a data esteja no formato YYYY-MM-DD esperado pela API
    ...
    try:
        while True:
            params = {
                ...
                "ordenarPor": "id",
                "pagina": pagina_atual,
                "itens": 100  # Aumentado para melhor performance com batch saving
            }
            
            # Timeout aumentado para 30s conforme solicitado
            ...
            for prop in dados:
                ...
                # Otimização: Preenchendo com 'A pesquisar' para evitar chamadas extras à API (fetch_detalhes removido)
                ...
                # Atualiza a data para o checkpoint (apenas a parte YYYY-MM-DD)
            ...
            # Batch Saving: Salva apenas ao final de cada página processada
